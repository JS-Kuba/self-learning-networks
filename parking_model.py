...
def final_score(phis_params, state, if_collision, num_of_steps):
    ...
    rational_num_of_steps = (phis_params.park_depth + phis_params.street_width + \
...
def random_initial_states(phis_params, num_of_states):
    ...
    while len(created_states) < num_of_states:
        x = 2*(np.random.rand() - 0.5)*(phis_params.street_length/2 - car_min_size)
        y = phis_params.street_width/2 + phis_params.car_width/2 +\
            2*(np.random.rand() - 0.5)*(phis_params.street_width/2 - car_min_size)
