...
def nagroda(stan,nowystan,F):
    ...
    # ..............................................
    # ..............................................
    # nagroda_za_stabilnosc = -abs(nowystan[2]) if abs(nowystan[0]) < np.pi / 6 else 0
    R = -(kara_za_odchylenie + kara_za_przewrocenie)# + nagroda_za_stabilnosc
    return R
...
def sila(stan, wagi):
    kod_stanu = np.array([stan[0], stan[1], stan[2], stan[3], 1])
    F = np.dot(kod_stanu, wagi) 
    Fmax = wah_glob()[0]
...
def wahadlo_uczenie():
    ...
    liczba_wag = 5       # na razie, by sie uruchomilo
    ...
    for epizod in tqdm(range(liczba_epizodow)):
        ...
        while (krok < 1000) & (czy_wahadlo_przewrocilo_sie == 0):
            ...
            Q_obecne = np.dot(np.array([stan[0], stan[1], stan[2], stan[3], 1]), wagi)
            Q_nastepne = np.dot(np.array([nowystan[0], nowystan[1], nowystan[2], nowystan[3], 1]), wagi)
            delta = R + gamma * Q_nastepne - Q_obecne
            wagi += alfa * delta * np.array([stan[0], stan[1], stan[2], stan[3], 1])
